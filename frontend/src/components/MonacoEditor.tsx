...
import { X, Save, Undo, Redo, Copy, Search, Replace, Users } from 'lucide-react';
...
export const MonacoEditor: React.FC<MonacoEditorProps> = ({
  ...
  useEffect(() => {
    if (user && roomId) {
      ...
      const handleRemoteChange = (change: any) => {
        console.log('🔄 MonacoEditor received remote change:', change);
        ...
        if (activeTab && activeTab.fileId === change.fileId) {
          ...
          if (model) {
            setTabs(prev => prev.map((tab, index) => 
              index === activeTabIndex ? { ...tab, content: model.getValue() } : tab
            ));
          }
        }
      };
      ...
      const handleCursorUpdate = (cursor: any) => {
        console.log('🎯 MonacoEditor received cursor update:', cursor);
        collaborationService.updateRemoteCursor(cursor);
        setCollaborators(new Map(collaborationService.getUserCursors()));
      };

      const handleFileSync = (data: any) => {
        collaborationService.initializeFile(data.fileId, data.content, data.version);
      ...
      const handleRequestFileSyncFromPeer = (data: { fileId: string; requesterId: string }) => {
  ...
  const handleEditorChange = useCallback((value: string | undefined) => {
    if (value === undefined || activeTabIndex < 0 || activeTabIndex >= tabs.length) return;
  ...
  const handleCloseTab = (tabIndex: number) => {
    ...
    setTabs(newTabs.map((t, i) => ({ ...t, isActive: i === (tabIndex === activeTabIndex ? Math.max(0, tabIndex - 1) : activeTabIndex) })));
  };
  ...
  const handleTabClick = (tabIndex: number) => {
    ...
    if (editorRef.current && newActiveTab && monaco) {
      ...
      collaborationService.initializeFile(newActiveTab.fileId, newActiveTab.content, 0);
        ...
        {tabs.map((tab, index) => (
          ...
          >
            <span className="truncate flex-1">
              {tab.name}
              {tab.isModified && <span className="text-discord-primary ml-1">●</span>}
            </span>
            <Button
              variant="ghost"
              size="sm"
            ...
            {collaborators.size > 0 && (
              <div className="flex items-center gap-1">
                ...
                <div className="flex gap-1">
                  {Array.from(collaborators.values()).slice(0, 3).map((cursor) => (
                    <Badge
                      key={cursor.userId}
                      variant="secondary"
                      className="text-xs px-1 py-0 h-5"
                      style={{ backgroundColor: cursor.color + '40', color: cursor.color }}
                    >
