...
import { registerAICompletionProvider } from "./aiCompletionProvider";
...
export const configureMonaco = (monaco: Monaco) => {
  // Define HiveCodex custom dark theme
  monaco.editor.defineTheme("hivecodex-dark", {
    base: "vs-dark",
    inherit: true,
    rules: [
      // Comments
    ...
    colors: {
      // Editor background - HiveCodex branding
  ...
  // Set the theme with error handling
  try {
    monaco.editor.setTheme("hivecodex-dark");
  } catch (themeError) {
    console.warn('Error setting Monaco theme:', themeError);
  ...
  // Add formatting providers for various languages
  ...
  // TODO: Re-enable AI completion provider after basic editor is working
